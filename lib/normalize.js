...
function objectifyMapping(mapping) {
  ...
  const destModel = toCompiledType(getDestAttributes(destComplete), key);
...
function getSrcAttributes(src) {
  ...
  const srcModel = {
    ...
    isFile: !!extname,
    isDirectory: !extname,
    sourceMappingURL: path.join(sourcemapsDir, path + '.map')
  };

  srcModel.sourcemapsDirname = path.join(destDir, sourcemapsDir, srcModel.dirname);
  srcModel.sourcemapsPath = path.join(srcModel.sourcemapsDirname, srcModel.dirname, srcModel.basename);
...
function toCompiledType(dest, key) {
  const typeMap = {
    scss: 'css',
    sass: 'css',
    less: 'css',
  ...
  if (compiledType) {
    ...
    dest.extnameMin = key + '.min' + dest.extname;
    dest.sourceMappingURLMin = path.join(sourcemapsDir, dest.extnameMin + '.map');
  }
...
function getWebPaths(optimize, webPaths, mapping) {
  if (optimize && !webPaths[mapping.key] && _.includes(['js', 'css'], mapping.dest.type)) {
    webPaths[mapping.key] = mapping.dest.extnameMin;
  }
